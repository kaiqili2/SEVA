...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            mask = mask.bool()
            # mask [batch size, seq_len, seq_len]
            padding_mask = mask.view(batch_size, 1, seq_len, seq_len)
...
class ClassificationHead(nn.Module):
    def __init__(self, emb_size: int = 768, n_classes: int = 3, other_feature_dim: int = 0, dropout: float = 0.2):
        super().__init__()
    ...
    def forward(self, x, mask, other_feature):
        ...
        if mask is not None:
            x = x.sum(1) / ((1 - mask[:, :, 1]).sum(1).unsqueeze(-1))
        else:
            x = x.mean(1)
...
class SEVA(nn.Module):
    def __init__(self,
        ...
        self.classification_layer = ClassificationHead(emb_size, n_classes, other_feature_dim, dropout)
